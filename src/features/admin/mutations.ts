...
export async function updateDealAction(formData: FormData) {
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      bundle_text: String(formData.get('bundleText') ?? '') || null,
      ...
      discount_percent: String(formData.get('discountPercent') ?? '') ? Number(formData.get('discountPercent')) : null,
      ...
      deal_type_id: String(formData.get('dealTypeId') ?? ''),
